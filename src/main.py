		...
		try:
			...
			if (dt.vk_code!=VK_PACKET and dt.flags&(LLKHF_INJECTED|LLKHF_ALTDOWN)!=LLKHF_INJECTED|LLKHF_ALTDOWN):
				if (dt.vk_code==0xa5 and _handle._ig_alt==True):
					...
					if (not cb((VK_KEYS[dt.vk_code] if dt.vk_code in VK_KEYS else None),dt.vk_code,(True if dt.vk_code in NUMPAD_VK else False),(True if wp in (WM_KEYDOWN,WM_SYSKEYDOWN) else False))):
						return -1
...
register_hook(lambda k,vk,np,p:(print(f"key={k}, vk={vk}, is_numpad={np}, pressed={p}"),True)[1])
